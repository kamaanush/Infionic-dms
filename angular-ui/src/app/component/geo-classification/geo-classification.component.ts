...
export class GeoClassificationComponent implements OnInit {
          ...
          if (index == -1) {
            if (hirerachyIndex == this.geoGraphyFullData.length) {
              ...
              this.geoGraphyFullData[
                hirerachyIndex - 1
              ].geographyNamesSelected.push(element.geographyName);
            } else {
  ...
  geoGraphyBreadCrumb(index) {
    ...
    for (var i = index - 2; i <= index - 1; i++) {
      if (
        this.geoGraphyFullData[i] &&
        this.geoGraphyFullData[i].geographyNamesSelected
      ) {
        breadCrumb +=
          this.geoGraphyFullData[i].geographyNamesSelected.join(',');
        if (i != index - 1) {
  ...
  activateDeactivateGeography(event, geoItem, hirerachyIndex) {
    ...
    console.log(geoItem);
    const dialogRef = this.dialog.open(GeoActivateDeactivateComponent, {
      disableClose: true,
      data: geoItem,
    });
  ...
  ActivateDeactivateGeography(geoItem, hirerachyIndex) {
    ...
    this.classification
      ...
      .subscribe({
        next: (res) => {
          if (
            res.response.result === 'Successfully Activated' ||
            res.response.result === 'Successfully Deactivated'
          ) {
            if (res.response.result.indexOf('Successfully') != -1) {
              let geoGraphyObj = this.geoGraphyFullData[
                hirerachyIndex - 1
              ].allOtherGeography.find(
                (x) => x.geographyId == geoItem.geographyId
              );
              if (geoGraphyObj) {
                geoGraphyObj.isActive = !geoGraphyObj.isActive;
                let geoGraphyIndex = this.geoGraphyFullData[
                  hirerachyIndex - 1
                ].geographySelected.indexOf(geoGraphyObj.geographyId);
                if (geoGraphyIndex != -1) {
                  this.geoGraphyFullData[
                    hirerachyIndex - 1
                  ].geographySelected.splice(geoGraphyIndex, 1);
                  this.removeOtherGeographiesData(hirerachyIndex);
                }

                this.dialog.open(GeoStatusPopComponent, {
                  panelClass: geoGraphyObj.isActive
                    ? 'activeSuccessPop'
                    : 'deactiveSuccessPop',
                  data: geoGraphyObj,
                });
